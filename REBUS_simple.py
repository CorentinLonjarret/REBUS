# Implementation of the REBUS with Markov chains
import pandas as pd
...
class REBUS_simple:
    def __init__(self, dataset, args):
        print('In class REBUS_simple')
        self.dataset = dataset
        self.args = args
        if not os.path.exists(os.path.join("tmp", "REBUS_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name)):
            os.makedirs(os.path.join("tmp", "REBUS_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name))
        random_id = str(random.randint(0, 1000000))
        self.path_saver_parameters = os.path.join("tmp", "REBUS_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name, random_id+".ckpt")
        args_dict = vars(args)
        with open(os.path.join("tmp", "REBUS_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name, random_id+"-args.json"), 'w') as fp:
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(self.args.max_iters):
                ...
                for k in range(0, num_complete_minibatches):
                    # for k in tqdm(range(num_complete_minibatches), total=num_complete_minibatches, ncols=70, leave=False, unit='b'):
