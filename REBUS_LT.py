# Implementation of the REBUS With only the long term part
import pandas as pd
...
class REBUS_LT:
    def __init__(self, dataset, args):
        print('In class REBUS_LT')
        self.dataset = dataset
        self.args = args
        if not os.path.exists(os.path.join("tmp", "REBUS_LT"+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name)):
            os.makedirs(os.path.join("tmp", "REBUS_LT"+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name))
        random_id = str(random.randint(0, 1000000))
        self.path_saver_parameters = os.path.join("tmp", "REBUS_LT"+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name, random_id+".ckpt")
        args_dict = vars(args)
        with open(os.path.join("tmp", "REBUS_LT"+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name, random_id+"-args.json"), 'w') as fp:
    ...
    def create_placeholder(self):
        pl_user_list = tf.placeholder(tf.int64, shape=[None], name='user_list')  # List of all users
        pl_list_prev_items_pos = tf.placeholder(tf.int64, shape=[None, self.args.max_lens], name='list_prev_item_pos')
        pl_list_prev_items_neg = tf.placeholder(tf.int64, shape=[None, self.args.max_lens], name='list_prev_item_neg')
        pl_pos = tf.placeholder(tf.int64, shape=[None], name='pos')
        pl_neg = tf.placeholder(tf.int64, shape=[None], name='neg')  # Shape for the neg sparseTensor
    ...
    def create_feed_dict(self, placeholders, users, prev_items, list_prev_items_pos, list_prev_items_neg, pos_items, neg_items):
        feed_dict = {
    ...
    def get_preds(self, placeholders, parameters):
        ...
        dist_pos = tf.subtract(sum_long_pos, item_pos)
        ...
        dist_neg = tf.subtract(sum_long_neg, item_neg)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(self.args.max_iters):
                ...
                mini_batch_idx, batch_size, users, prev_items, list_prev_items_pos, list_prev_items_neg, pos_items, neg_items = self.dataset.generate_train_shuffled_batch_sp_with_prev_items()
                ...
                for k in range(0, num_complete_minibatches):
                    ...
                    feed_dict = self.create_feed_dict(self.placeholders, mini_batch_users, mini_batch_prev_items, mini_batch_list_prev_items_pos, mini_batch_list_prev_items_neg, mini_batch_pos_items, mini_batch_neg_items)
