# Implementation of the REBUS with only the short term part and Markov Chains
import pandas as pd
...
class REBUS_ST_simple:
    def __init__(self, dataset, args):
        print('In class REBUS_ST_simple')
        self.dataset = dataset
        self.args = args
        if not os.path.exists(os.path.join("tmp", "REBUS_ST_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name)):
            os.makedirs(os.path.join("tmp", "REBUS_ST_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name))
        random_id = str(random.randint(0, 1000000))
        self.path_saver_parameters = os.path.join("tmp", "REBUS_ST_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name, random_id+".ckpt")
        args_dict = vars(args)
        with open(os.path.join("tmp", "REBUS_ST_simple_"+str(self.args.L)+"_"+str(self.args.user_min)+"_"+str(self.args.item_min), self.dataset.data_name, random_id+"-args.json"), 'w') as fp:
